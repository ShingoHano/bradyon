...
- (void)loadData
{
    ...
    titleString=[userDefaults stringForKey:@"TITLE2"];
    titleString=titleString?titleString:@"tab2";            //NULL化されていた場合は"デフォルトタブ名を設定
    urlString=[userDefaults stringForKey:@"URL2"];
    ...
    reloadTime=[userDefaults integerForKey:@"RELOAD2"];
...
- (void)saveData:(int)index
{
    ...
    if(index==0)
        [userDefaults setObject:titleString forKey:@"TITLE2"];
    else if(index==1)
        [userDefaults setObject:urlString forKey:@"URL2"];
    else if(index==2)
        [userDefaults setInteger:reloadTime forKey:@"RELOAD2"];
